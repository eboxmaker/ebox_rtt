...
DateTime::DateTime(int y,int mon,int d,int h,int m,int s ,int utc_offset)
{
    err = 0;
    year = y;
    month = mon;
    ...
    minute = m;
    second =s;
    if(!DateTime::limitCheck(*this))//check and limit para
    {
        err = 0;
    }
}
...
bool DateTime::parse(int y,int mon,int d,int h,int m,int s ,int utc_offset)
{
    err = 0;
    year = y;
    month = mon;
...
bool DateTime::check(DateTime &dt)
{
    if(dt.second < 0)       return false;
    else if(dt.second > 59) return false;
    if(dt.minute < 0)       return false;
    else if(dt.minute > 59) return false;
    if(dt.hour < 0)         return false;
    else if(dt.hour > 23)   return false;

    if(dt.year < 0)         return false;
    else if(dt.year > 3000) return false;
    if(dt.month < 1)        return false;
    else if(dt.month > 12)  return false;
    
    if(dt.day < 1)          return false;
    else if(dt.day > days_in_month(dt.year,dt.month)) return false;
    return true;
}
...
String DateTime::toString(TimeFormat_t format ,TimeSeparatorFormat_t sep)
{
    ...
    switch((int)format)
    {
        ...
        case YY_MM_DD:
            snprintf(buf,32,"%02d%s%02d%s%02d",year%100,snap[sep][0],month,snap[sep][1],day,snap[sep][2]);break;
        case YYYY_MM_DD:
            snprintf(buf,32,"%04d%s%02d%s%02d",year,snap[sep][0],month,snap[sep][1],day,snap[sep][2]);break;
        case HH_MM_SS:
            snprintf(buf,32,"%02d%s%02d%s%02d",hour,snap[sep][3],minute,snap[sep][4],second,snap[sep][5]);break;
        case H12_MM_SS:
            am = (hour <= 12)?true:false;
            if(am)
            {
