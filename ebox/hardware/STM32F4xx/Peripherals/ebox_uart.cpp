...
int Uart::peek(void)
{
    if ((_rx_buffer_size[index] - hdma_rx.Instance->NDTR) == _rx_buffer_tail[index])
    {
        return -1;
    }
    else
...
{
    if(mode == RxDMA)
        return ((unsigned int)(_rx_buffer_size[index] + (_rx_buffer_size[index] - hdma_rx.Instance->NDTR) - _rx_buffer_tail[index])) % _rx_buffer_size[index];
    else
...
int Uart::read()
{
    if(mode == RxDMA)
    {
        if (_rx_buffer_tail[index] == (_rx_buffer_size[index] - hdma_rx.Instance->NDTR))
        {
            return -1;
        }
    }
