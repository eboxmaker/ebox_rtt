...
#if defined (STM32F10X_HD)
    #define UART_NUM (USE_UART1 + USE_UART2 + USE_UART3 + USE_UART4 + USE_LPUART1 + USE_UART5)
#else
    #define UART_NUM (USE_UART1 + USE_UART2 + USE_UART3 + USE_UART4 + USE_LPUART1)
#endif
...
{
public:
    Uart(USART_TypeDef *USARTx, Gpio *tx_pin, Gpio *rx_pin, uint16_t tx_buffer_size = 1024, uint16_t rx_buffer_size = 1024);
