...
/*液晶屏的分辨率，用来计算地址偏移*/
uint16_t lcd_width=640, lcd_height=480;
...
void OV2640_Init(void) 
{
    ...
    DCMI_Handle.Init.JPEGMode = ENABLE;
...
void OV2640_DMA_Config_buffer(uint32_t DMA_Memory0BaseAddr,uint32_t DMA_BufferSize)
{
    ...
    DMA_Handle_dcmi.Init.PeriphDataAlignment = DMA_PDATAALIGN_WORD;
    DMA_Handle_dcmi.Init.MemDataAlignment = DMA_MDATAALIGN_BYTE;
    DMA_Handle_dcmi.Init.Mode = DMA_NORMAL;								//循环模式
    DMA_Handle_dcmi.Init.Priority = DMA_PRIORITY_LOW;
    DMA_Handle_dcmi.Init.FIFOMode = DMA_FIFOMODE_ENABLE;
    DMA_Handle_dcmi.Init.FIFOThreshold = DMA_FIFO_THRESHOLD_FULL;
    DMA_Handle_dcmi.Init.MemBurst = DMA_MBURST_SINGLE;
...
uint8_t ov2640_start()
{
	uint16_t i;
	uint16_t reg;
	OV2640_WriteReg(OV2640_DSP_RA_DLMT, 0x01);	//操作sensor寄存器
 	OV2640_WriteReg(OV2640_SENSOR_COM7, 0x80);	//软复位OV2640
	HAL_Delay(50); 
	reg=OV2640_ReadReg(OV2640_SENSOR_MIDH);	//读取厂家ID 高八位
	reg<<=8;
	reg|=OV2640_ReadReg(OV2640_SENSOR_MIDL);	//读取厂家ID 低八位

//    rt_kprintf("0X%04X",reg); 

 	//初始化 OV2640,采用SXGA分辨率(1600*1200)  
	for(i=0;i<sizeof(ov2640_uxga_init_reg_tbl)/2;i++)
	{
	   	OV2640_WriteReg(ov2640_uxga_init_reg_tbl[i][0],ov2640_uxga_init_reg_tbl[i][1]);
 	} 
  	return 0x00; 	//ok
}
...
//	uint8_t isJpeg = 0;
	...
	OV2640_WriteReg(0XFF,0X00);
	OV2640_WriteReg(0XD3,30);
	OV2640_WriteReg(0XFF,0X01);
	OV2640_WriteReg(0X11,0X1); 
...
void OV2640_JPEGConfig(ImageFormat_TypeDef ImageFormat)
{
  ...
  switch(ImageFormat)
  {
		...
		case JPEG_1280x960:
    {
      ...
      {
        OV2640_WriteReg(OV2640_1280x960_JPEG[i][0], OV2640_1280x960_JPEG[i][1]);
      }
      break;
    }
