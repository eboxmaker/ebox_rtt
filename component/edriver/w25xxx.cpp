...
int W25xxx::init()
{
    ...
    if(initialized == 0)
    {
        ...
        if(ret == 0)
        {
            ...
            w25xDebug("cap        : %d MByte\n",(int)(capacity/1024.0/1024.0));
            w25xDebug("sct_size   : %d Byte\n",sector_size);
            w25xDebug("sct_count  : %d\n",sector_count);
            w25xDebug("================================\n");
            initialized = 1;
        }
    }
...
void W25xxx::write(uint8_t *buf, uint32_t write_addr, uint16_t num_to_write)
{
    ...
    uint8_t* spi_flash_buf = (uint8_t*)rt_malloc(4096);
